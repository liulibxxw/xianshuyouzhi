...
import { generateNativeCover } from './nativeExport';
...
const App: React.FC = () => {
  ...
  const handleExport = async (filename?: string) => {
    ...
    try {
        ...
        if (Capacitor.isNativePlatform()) {
            try {
                const base64 = await generateNativeCover(state);
                setExportImage(base64); // Show preview in modal
                setIsExporting(false);
                return;
            } catch (nativeErr) {
                console.error("Native render failed, falling back to web render", nativeErr);
            }
        }

        // Web Export Path (html-to-image)
  ...
  const downloadImage = async () => {
    ...
    const fileName = exportFilename || `cover_${Date.now()}.png`;
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        try {
          // Attempt to write to Documents (Android specific mostly)
          await Filesystem.writeFile({
            path: fileName,
            data: base64Data,
            directory: Directory.Documents,
          });
          showToast(`已保存至文档目录`, "success");
        } catch (fileError) {
