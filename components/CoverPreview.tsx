...
import React, { useEffect, useState, forwardRef, useRef, useMemo, useCallback, useLayoutEffect } from 'react';
...
const CoverPreview = forwardRef<HTMLDivElement, CoverPreviewProps>(({ state, onBodyTextChange, isExporting = false, longTextMinHeight = 0 }, ref) => {
  ...
  const isJianghuTheme = false;
  const effectiveCategory = category;

  const getTextAlignValue = (alignClass: string): React.CSSProperties['textAlign'] => {
    switch (alignClass) {
      case 'text-left':
        return 'left';
      case 'text-center':
  ...
  const bodyTextAlignStyle = useMemo<React.CSSProperties>(() => ({
    textAlign: getTextAlignValue(bodyTextAlign),
  }), [bodyTextAlign]);

  const bodyWrapStyle = useMemo<React.CSSProperties>(() => ({
    ...bodyTextAlignStyle,
  ...
  const [storybookMeasuredLineHeightPx, setStorybookMeasuredLineHeightPx] = useState<number>(28);
  ...
  const renderingIsolation: React.CSSProperties = {
    ...
    textAlign: getTextAlignValue(bodyTextAlign), // 确保文本对齐方式在导出时生效
    // 强制隔离渲染环境
    width: '400px',
    flexShrink: 0,
  ...
  // 导出与编辑统一使用同一份 HTML，避免导出阶段二次“清洗”造成文本结构漂移
  const displayBodyText = useMemo(() => bodyText, [bodyText]);
  ...
  // Android WebView 下字体度量与桌面浏览器可能不一致：
  // 使用真实 computed line-height 驱动横线步进，避免“网页正常、APK 错位”。
  useLayoutEffect(() => {
    if (layoutStyle !== 'storybook') return;
    ...
    const measureLineHeight = () => {
      ...
      if (!Number.isNaN(parsed) && parsed > 0) {
        setStorybookMeasuredLineHeightPx(Math.round(parsed * 100) / 100);
      }
    };

    measureLineHeight();

    // 字体/视口变化后重复测量一段时间，覆盖 Android 异步字体加载场景
    let rafId = 0;
    let tick = 0;
    const loopMeasure = () => {
      measureLineHeight();
      tick += 1;
      if (tick < 24) rafId = requestAnimationFrame(loopMeasure);
    };
    rafId = requestAnimationFrame(loopMeasure);

    const onResize = () => measureLineHeight();
    window.addEventListener('resize', onResize);
    window.visualViewport?.addEventListener('resize', onResize);

    const timer = window.setInterval(measureLineHeight, 300);
  ...
  }, [layoutStyle, bodyTextSize, bodyFont, mode, subtitle, title]);
  ...
  const bodyFontSizePx = useMemo(() => {
    const match = bodyTextSize.match(/text-\[(\d+(?:\.\d+)?)px\]/);
    return match ? parseFloat(match[1]) : 13;
  }, [bodyTextSize]);

  const storybookLineHeightPx = useMemo(() => {
    if (storybookMeasuredLineHeightPx > 0) return storybookMeasuredLineHeightPx;
    return Math.round(bodyFontSizePx * 1.5 * 100) / 100;
  }, [storybookMeasuredLineHeightPx, bodyFontSizePx]);
  
    const getBodyClasses = () => {
      return `${bodyTextSize} ${bodyTextAlign} leading-[1.5] outline-none ${getBodyFontClass()} editor-break`;
    };
    ...
    const getStorybookBodyHeightClass = () => {
        ...
        if (isExporting) {
          return 'h-auto overflow-visible';
        }
        // 长文初始态需要铺满可视区域，同时允许内容继续向下增长
        return 'min-h-full h-auto overflow-visible';
    };
  ...
  const AutoFitSubtitle: React.FC<{
    ...
    maxWidth: number; // 最大可用宽度（会与容器实时宽度取较小值）
    contentKey?: string;
    syncWidthRef?: React.RefObject<HTMLElement>;
  }> = useCallback(({ children, className = '', style = {}, maxWidth, contentKey = '', syncWidthRef }) => {
    const containerRef = useRef<HTMLDivElement>(null);
    const innerRef = useRef<HTMLSpanElement>(null);
    const [fontSize, setFontSize] = useState<number | undefined>(undefined);
    const baseFontSizeRef = useRef<number>(0);
    const lastStableWidthRef = useRef<number>(0);
    const [resolvedWidth, setResolvedWidth] = useState<number>(maxWidth);
    ...
    useLayoutEffect(() => {
      ...
      const updateWidth = () => {
        const measured = container.clientWidth || maxWidth;
        const targetMeasured = syncWidthRef?.current?.clientWidth;
        const effective = targetMeasured && targetMeasured > 0
          ? Math.min(measured, targetMeasured)
          : Math.min(maxWidth, measured);
        setResolvedWidth(prev => (Math.abs(prev - effective) > 1 ? effective : prev));
      };
      ...
      if (typeof ResizeObserver !== 'undefined') {
        ...
        if (syncWidthRef?.current) {
          observer.observe(syncWidthRef.current);
        }
      } else {
        // Android WebView 某些版本不稳定支持 ResizeObserver，使用短轮询兜底
        fallbackTimer = window.setInterval(updateWidth, 250);
      }
    ...
    useLayoutEffect(() => {
      ...
      const runMeasure = () => {
        const availableWidth = Math.min(resolvedWidth, container.clientWidth || resolvedWidth);
        if (availableWidth <= 0) return;
        if (availableWidth < 40) return;

        if (!lastStableWidthRef.current) {
          lastStableWidthRef.current = availableWidth;
        }

        // 临时移除内联字号，测量“基准字号”下的自然宽度；在 useLayoutEffect 中执行可避免可见闪烁
        const previousInlineSize = inner.style.fontSize;
        inner.style.fontSize = '';

        // WebView 下 scrollWidth 可能偶发不稳定，使用双通道取最大值
        const naturalWidth = Math.max(inner.scrollWidth, inner.getBoundingClientRect().width);
        const computedSize = parseFloat(getComputedStyle(inner).fontSize) || 14;
        if (!baseFontSizeRef.current || baseFontSizeRef.current < computedSize) {
          baseFontSizeRef.current = computedSize;
        }
        const baseFontSize = baseFontSizeRef.current || computedSize;

        // 还原原有内联字号，最终由 React state 统一控制
        inner.style.fontSize = previousInlineSize;

        const needsShrink = naturalWidth > availableWidth + 1;

        const measureWidthAt = (size: number) => {
          const prevInline = inner.style.fontSize;
          inner.style.fontSize = `${size}px`;
          const measuredWidth = Math.max(inner.scrollWidth, inner.getBoundingClientRect().width);
          inner.style.fontSize = prevInline;
        ...
        if (needsShrink) {
          ...
          for (let i = 0; i < 10; i++) {
            ...
            const widthAtMid = measureWidthAt(mid);
            if (widthAtMid <= availableWidth - 0.5) {
              low = mid;
            } else {
              high = mid;
            }
          ...
          const nextSize = Math.max(6, Math.min(baseFontSize, low));
          setFontSize(prev => (prev !== undefined && Math.abs(prev - nextSize) < 0.2 ? prev : nextSize));
          lastStableWidthRef.current = availableWidth;
          return;
        }

        // Web 环境不强制写内联字号，避免切换面板时副标题闪烁；
        // Native(WebView)继续保留明确字号，避免 class 回写覆盖导致截断。
        if (isNativeApp) {
          const widthDelta = Math.abs(availableWidth - lastStableWidthRef.current);
          // 避免 Android 面板动画造成瞬时宽度抖动时的“缩小/回弹”闪烁
          if (widthDelta > 6) {
            lastStableWidthRef.current = availableWidth;
            return;
          }
          setFontSize(prev => (prev !== undefined && Math.abs(prev - baseFontSize) < 0.35 ? prev : baseFontSize));
        } else {
          setFontSize(prev => (prev === undefined ? prev : undefined));
        }
        lastStableWidthRef.current = availableWidth;
      };

      // 多轮测量：覆盖 Android WebView 字体异步加载/首次布局不稳定问题
      const rafIds: number[] = [];
      const timers: number[] = [];

      const scheduleMeasurePasses = () => {
        rafIds.push(requestAnimationFrame(runMeasure));
        [60, 180, 400, 800, 1400].forEach((delay) => {
      ...
      // Native WebView 字体加载经常延迟且分批生效，追加短时重测保证最终不截断
      const nativeStabilizeTimers: number[] = [];
      if (isNativeApp) {
        [280, 720, 1300, 2100].forEach((delay) => {
          nativeStabilizeTimers.push(window.setTimeout(runMeasure, delay));
        });
      }
      ...
      return () => {
        ...
        nativeStabilizeTimers.forEach(id => window.clearTimeout(id));
      };
    }, [contentKey, resolvedWidth, isNativeApp]);
      ...
      >
        <span
          ...
          style={{
            ...
            whiteSpace: 'nowrap',
            // 不限制内部自然宽度，确保在 Android WebView 中可准确测得真实文本宽度
                ...
                <div className="flex items-center mb-2 shrink-0">
                 ...
                 >
                    ...
                    <div
                      ...
                      style={{ color: textColor, ...bodyWrapStyle }}
                    />
                </div>
              </div>
          </div>
      ...
      return (
        ...
        >
          ...
          <div className={`storybook-border relative flex flex-col ${isLongText ? 'flex-1' : flexGrowClass} ${minHeightClass}`} style={{ backgroundColor: earthyColors.cream, overflow: 'hidden' }}>
            ...
            {/* Body text area */}
            <div 
              className={`relative mx-4 mt-2 ${isLongText ? 'flex-1 min-h-0' : `${flexGrowClass} ${minHeightClass}`} cursor-text`}
              onClick={handleContainerClick}
            >
              ...
              <div
                ...
                style={{ 
                  ...
                  ...bodyWrapStyle,
                  lineHeight: '1.5',
                  // 横线步进与正文行高完全一致，避免网格与文本脱节
                  backgroundImage: `repeating-linear-gradient(to bottom, transparent 0, transparent ${Math.max(storybookLineHeightPx - 1, 0)}px, ${earthyColors.brown}1A ${Math.max(storybookLineHeightPx - 1, 0)}px, ${earthyColors.brown}1A ${storybookLineHeightPx}px)`,
